...
{
    public partial class AuthHospitalService
    {
        ...
        public ResponseModel SaveAuthMini(JObject jsonobj)
        {
            ...
            {
                return SaveMini(myEmployeeID, hospitalID, hospitalName, procuctID, productName, grid, gridName, autherID, autherName, viewerID, viewerName, saleMode, saleModeName);
            }
            else
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    for (int i = 0; i < formID.Split('|').Length; i++)
                    {
                        ...
                        sql = $"Select *  From [yaodaibao].[dbo].[formmain_8916] where FormID = '{formID.Split('|')[i]}' and AuthState = '{j}'";
                    ...
                    {
                        //如果下一个审核人是自己 ， 审批结束
                        sql = $"SELECT FormID FROM yaodaibao.dbo.formmain_8916 WHERE NextAuthID = '{myEmployeeID}'";
                        dataTable = serverHelper.ExecuteSql(sql);
                        ...
                        {
                            AuditMini(myEmployeeID, string.Join("|", formIDList.ToArray()), string.Join("|", checkStateList.ToArray()));
                        }
                    }
                    //审批
                    else if (j == 1)
        ...
        /// </summary>
        /// <returns></returns>
        public ResponseModel GetAuthDataMini(string employeeID, string queryState = "", string auther = "", string hospital = "")
        {
            SQLServerHelper serverHelper = new();
            string sql = "";
        ...
        /// <returns></returns>
        public ResponseModel SaveMini(string myEmployeeID, string hospitalID, string hospitalName, string ProcuctID, string ProductName, string grid, string gridName, string autherID, string autherName, string viewerID, string viewerName, string saleMode, string saleModeName)
        {
            SQLServerHelper serverHelper = new();
        ...
        /// 审核
        /// </summary>
        /// <param name="myEmployeeID"></param>
        /// <param name="formID"></param>
        /// <param name="checkState"></param>
        /// <returns></returns>
        public ResponseModel AuditMini(string myEmployeeID, string formID, string checkState)
        {
            SQLServerHelper serverHelper = new();
        ...
        /// <param name="checkState"></param>
        /// <returns></returns>
        public ResponseModel ApproveMini(string myEmployeeID, string formID, string hospitalID, string productID, string productName, string autherID, string viewerID, string gridID, string checkState)
        {
