...
namespace ydb.Domain.Service
{
    public class RecordHospitalService : IRecordHospitalService
    {
        public ResponseModel CheckRecord(QueryRecordModel recordModel)
        {
           ...
           string sql = $"select  t1.* , t2.FName fieldrecordname  from [yaodaibao].[dbo].[formson_8927] t1 left join   [yaodaibao].[dbo].[t_items] t2 on t1.field0007 = t2.FID where t1.field0009='{recordModel.field0009}' and t1.field0010='{recordModel.field0010}'";
        ...
        public ResponseModel EditRecordHospital(QueryRecordModel recordModel)
        {
            ...
            SQLServerHelper sqlServerHelper = new SQLServerHelper();
            DataTable dataTable = null;
           string sql = $"select  t1.*,t2.FName fieldrecordname from [yaodaibao].[dbo].[formson_8927] t1 left join   [yaodaibao].[dbo].[t_items] t2 on t1.field0007 = t2.FID where t1.FID='{recordModel.Id}'";
            dataTable = sqlServerHelper.ExecuteSql(sql);
            if (dataTable.Rows.Count > 0)
            {
                foreach (DataRow row in dataTable.Rows)
        ...
        public ResponseModel GetMyRecordingHospital(QueryRecordModel recordModel)
        {
            ...
            SQLServerHelper sqlServerHelper = new SQLServerHelper();
            DataTable dataTable = null;
            string sql = $"select  t1.* ,t3.FName fieldrecordname from [yaodaibao].[dbo].[formson_8927] t1 left join [yaodaibao].[dbo].[formmain_8926] t2 on t1.FID=t2.FID left join [yaodaibao].[dbo].[t_items] t3 on t1.field0007 = t3.FID where t2.field0001='{recordModel.employeeID}' and t1.field0037='开发中'";
            dataTable = sqlServerHelper.ExecuteSql(sql);
            if (dataTable.Rows.Count > 0)
            {
                foreach (DataRow row in dataTable.Rows)
        ...
        public ResponseModel QueryRecordHospital(QueryRecordModel recordModel)
        {
            ...
            DataTable dataTable = null;
            string sql = $"select  t1.* , t3.FName fieldrecordname from [yaodaibao].[dbo].[formson_8927] t1 left join [yaodaibao].[dbo].[formmain_8926] t2 on t1.FID=t2.FID  left join [yaodaibao].[dbo].[t_items] t3 on t1.field0007 = t3.FID where t2.field0001 in('{subids}')";
            dataTable = sqlServerHelper.ExecuteSql(sql);
            if (dataTable.Rows.Count > 0)
            {
                foreach (DataRow row in dataTable.Rows)
        ...
        public ResponseModel SaveRecordHospital(SaveRecordHospital saveRecordHospital)
        {
            ...
            for (int i = 0; i < saveRecordHospital.field0004.Split("|").Length; i++)
            {
                ...
                sql = "";
                //新增
                if (saveRecordHospital.saveState.Split("|")[i]=="0")
                {
                ...
                {
                    sql = $"update [yaodaibao].[dbo].[formson_8927] set field0004='{saveRecordHospital.field0004.Split("|")[i > saveRecordHospital.field0004.Split("|").Length ? saveRecordHospital.field0004.Split("|").Length : i]}' , field0009='{saveRecordHospital.field0009.Split("|")[i > saveRecordHospital.field0009.Split("|").Length ? saveRecordHospital.field0009.Split("|").Length : i] }' ,  field0010='{saveRecordHospital.field0010.Split("|")[i > saveRecordHospital.field0010.Split("|").Length ? saveRecordHospital.field0010.Split("|").Length : i]}' ,  field0005='{saveRecordHospital.field0005.Split("|")[i > saveRecordHospital.field0005.Split("|").Length ? saveRecordHospital.field0005.Split("|").Length : i]}' ,  field0007='{saveRecordHospital.field0007.Split("|")[i > saveRecordHospital.field0007.Split("|").Length ? saveRecordHospital.field0007.Split("|").Length : i]}' ,  field0008='{saveRecordHospital.field0008.Split("|")[i > saveRecordHospital.field0008.Split("|").Length ? saveRecordHospital.field0008.Split("|").Length : i]}' ,  field0037='{saveRecordHospital.field0037.Split("|")[i > saveRecordHospital.field0037.Split("|").Length ? saveRecordHospital.field0037.Split("|").Length : i]}' ,  field0036='{saveRecordHospital.field0036.Split("|")[i > saveRecordHospital.field0036.Split("|").Length ? saveRecordHospital.field0036.Split("|").Length : i]}' ,  fieldvalid='{saveRecordHospital.fieldvalid.Split("|")[i > saveRecordHospital.fieldvalid.Split("|").Length ? saveRecordHospital.fieldvalid.Split("|").Length : i]}' ,  fieldinvalid='{saveRecordHospital.fieldinvalid.Split("|")[i > saveRecordHospital.fieldinvalid.Split("|").Length ? saveRecordHospital.fieldinvalid.Split("|").Length : i]}' ,  fielddes='{saveRecordHospital.fieldDes.Split("|")[i > saveRecordHospital.fieldDes.Split("|").Length ? saveRecordHospital.fieldDes.Split("|").Length : i]}' ,  fieldbakemployee='{saveRecordHospital.fieldbakemployee.Split("|")[i > saveRecordHospital.fieldbakemployee.Split("|").Length ? saveRecordHospital.fieldbakemployee.Split("|").Length : i]}',  fieldbakemployee='{saveRecordHospital.fieldbakemployeeid.Split("|")[i > saveRecordHospital.fieldbakemployeeid.Split("|").Length ? saveRecordHospital.fieldbakemployeeid.Split("|").Length : i]}' where FID='{saveRecordHospital.Id.Split("|")[i]}'";
                    sqlServerHelper.ExecuteSql(sql);
                }
            }
