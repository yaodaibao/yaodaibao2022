using CashBase.Common;
using CashBase.IRepository.Base;
using CashBase.IRepository.UnitOfWork;
using SqlSugar;
using System;
...
using CashBase.Common.DB;
using CashBase.IServices.Repository;

namespace CashBase.Repository.Base
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        private ISqlSugarClient _db
        {
            get
            {
                ...
                {
                    if (SqlSugar.ReflectionExtensions.GetTypeInfo(typeof(TEntity)).GetCustomAttributes(typeof(SugarTable), true).FirstOrDefault((x => x.GetType() == typeof(SugarTable))) is SugarTable sugarTable && !string.IsNullOrEmpty(sugarTable.TableDescription))
                    {
